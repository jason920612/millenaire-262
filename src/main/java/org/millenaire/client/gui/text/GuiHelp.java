...
public class GuiHelp extends GuiText {
   ...
   int helpDisplayed = 1;
   Identifier background = Identifier.fromNamespaceAndPath("millenaire", "textures/gui/help.png");
   ...
   protected void customDrawScreen(int mouseX, int mouseY, float f) {
      ...
      if (mouseX > 0 && mouseX < 32) {
         int pos = mouseY / 32;
         if (pos >= 0 && pos < 13) {
            int stringlength = this.fontRenderer.width(LanguageUtilities.string("help.tab_" + (pos + 1)));
            this.drawGradientRect(mouseX + 10 - 3, mouseY + 10 - 3, mouseX + 10 + stringlength + 3, mouseY + 10 + 14, -1073741824, -1073741824);
            this.drawString(LanguageUtilities.string("help.tab_" + (pos + 1)), mouseX + 10, mouseY + 10, 9474192);
         }
      }
      ...
      if (mouseX > 224 && mouseX < 256) {
         int pos = mouseY / 32;
         if (pos >= 0 && pos < 6) {
            int stringlength = this.fontRenderer.width(LanguageUtilities.string("help.tab_" + (pos + 8)));
            this.drawGradientRect(mouseX + 10 - 3, mouseY + 10 - 3, mouseX + 10 + stringlength + 3, mouseY + 10 + 14, -1073741824, -1073741824);
            this.drawString(LanguageUtilities.string("help.tab_" + (pos + 8)), mouseX + 10, mouseY + 10, 9474192);
         }
      }
   }
   ...
   @Override
   protected void millMouseClicked(int mouseX, int mouseY, int k) {
      int xStart = (this.width - this.getXSize()) / 2;
      int yStart = (this.height - this.getYSize()) / 2;
      ...
      super.millMouseClicked(mouseX, mouseY, k);
   }
}
