...
public class GoalGenericTakeFromBuilding extends GoalGeneric {
   ...
   @ConfigAnnotations.FieldDocumentation(
      explanation = "Minimum number of items to gather in one go."
   )
   public int minimumpickup;
   @ConfigAnnotations.ConfigField(
      type = AnnotedParameter.ParameterType.INVITEM_NUMBER_ADD,
      paramName = "collect_good"
   )
   @ConfigAnnotations.FieldDocumentation(
      explanation = "Goods to be picked up from the target building, with maximum quantity to have in inventory."
   )
