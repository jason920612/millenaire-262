 ...
 * Mirrors the 1.12 algorithm: an oak trunk, a large rounded pistachio-leaf canopy and branches.
 */
public class WorldGenPistachio extends MillTreeGenerator {
   private static final int MIN_TREE_HEIGHT = 6;
   private static final BlockState WOOD_BS = Blocks.OAK_LOG.defaultBlockState();
   private static final BlockState LEAVES_BS = MillBlocks.LEAVES_PISTACHIO.defaultBlockState()
   ...
   public boolean generate(Level worldIn, Random rand, BlockPos position) {
      ...
      for (int yPos = baseY + 3; yPos <= baseY + treeHeight; yPos++) {
         int leavesRadius = 4;
         if (yPos < baseY + 5) {
            leavesRadius -= baseY + 5 - yPos;
         } else if (yPos > baseY + treeHeight - 3) {
            leavesRadius -= yPos - (baseY + treeHeight - 3);
         }
