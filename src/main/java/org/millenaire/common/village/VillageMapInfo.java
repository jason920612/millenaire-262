...
public class VillageMapInfo implements Cloneable {
         ...
         for (int i = 0; i < this.length; i += 16) {
            for (int j = 0; j < this.width; j += 16) {
               try {
                  ...
                  MillLog.error(this, "LevelChunk access violation while updating map info.");
               }
            }
         }
   ...
   public boolean update(Level world, List<BuildingLocation> locations, Point centre, int radius) throws MillLog.MillenaireException {
      this.world = world;
   ...
   private void updateChunk(int startX, int startZ) throws ThreadSafeUtilities.ChunkAccessException {
         ...
         for (int deltaZ = -1; deltaZ < 2; deltaZ++) {
            if (!ThreadSafeUtilities.isChunkAtLoaded(this.world, startX + this.mapStartX + deltaX * 16, startZ + this.mapStartZ + deltaZ * 16)) {
               if (MillConfigValues.LogWorldInfo >= 3) {
                  MillLog.debug(this, "LevelChunk is not loaded at: " + (startX + this.mapStartX + deltaX * 16) + "/" + (startZ + this.mapStartZ + deltaZ * 16));
               }
      ...
      LevelChunk chunk = this.world.getChunkAt(new BlockPos(startX + this.mapStartX, 0, startZ + this.mapStartZ));
      if (MillConfigValues.LogWorldInfo >= 3) {
         MillLog.debug(this, "Updating chunk: " + startX + "/" + startZ + "/" + this.yBaseline + "/" + chunk.getPos().x() + "/" + chunk.getPos().z());
      }
      ...
      for (int i = 0; i < 16; i++) {
         for (int j = 0; j < 16; j++) {
            ...
            int mx = i + startX;
            int mz = j + startZ;
            int wx = mx + this.mapStartX;
            int wz = mz + this.mapStartZ;
            ...
            for (Block tblock = chunk.getBlockState(new BlockPos(wx, maxy, wz)).getBlock();
               y >= miny && !BlockItemUtilities.isBlockGround(tblock);
               tblock = chunk.getBlockState(new BlockPos(wx, --y, wz)).getBlock()
            ) {
            ...
            Block block;
            if (y <= maxy && y > 1) {
               block = chunk.getBlockState(new BlockPos(wx, y, wz)).getBlock();
            } else {
               block = null;
            }
            ...
            while (
               y <= maxy
                  && y > 1
                  && (
                     BlockItemUtilities.isBlockSolid(chunk.getBlockState(new BlockPos(wx, y, wz)).getBlock())
                        || BlockItemUtilities.isBlockSolid(chunk.getBlockState(new BlockPos(wx, y + 1, wz)).getBlock())
                  )
            ) {
               y++;
            }
            ...
            Block soilBlock = chunk.getBlockState(new BlockPos(wx, y - 1, wz)).getBlock();
            block = chunk.getBlockState(new BlockPos(wx, y, wz)).getBlock();
            if (BlockItemUtilities.isBlockWater(block)) {
               this.water[mx][mz] = true;
            }

            if (soilBlock.defaultBlockState().is(net.minecraft.tags.BlockTags.LOGS)) {
      ...
      public void run() {
         try {
            ...
            if (MillConfigValues.LogChunkLoader >= 2) {
               MillLog.minor(
                  this,
                  "LevelChunk access violation while attempting to update village map in thread at: "
